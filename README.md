<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=36&duration=3000&color=6AAFE6&center=true&vCenter=true&width=800&lines=Hey+There+👋" />
</p>
<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=28&duration=2500&color=6AAFE6&center=true&vCenter=true&width=700&lines=I'm+Amirmohammad+Kazemi;Frontend+Developer+🚀" />
</p>

---

## 🚀 About Me

- **Frontend Developer | React Specialist**
- Building **scalable, real-time & map-based web applications**
- Experienced in **dashboards, admin panels & multi-role systems**
- Focused on **performance, clean architecture & maintainability**
- Passionate about creating **real-world production systems**

---

## 🧠 Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,html,css,tailwind,bootstrap,git,github,vite,figma&theme=dark" />
</p>

---

## 💼 Featured Projects

### 🚚 Tak30 – Delivery Platform
- Full-scale **multi-role system** (user, courier, operator)
- Real-time courier tracking with frequent updates
- Dynamic routing (courier → pickup → destination)
- Optimized map-based UI components

**Tech:** React, Map.ir, Leaflet, Tailwind  

---

### 🛍️ Bazarcheh.de – E-commerce Platform (Germany)
- Advanced **product management system** with admin panel
- Multi-step product workflows (create/edit)
- Scalable forms with validation & media upload
- Improved UI consistency across admin dashboard

**Tech:** React, React Hook Form, Bootstrap  

---

### 👕 Dehkade Arzani – Clothing Store
- Modern e-commerce frontend built with Next.js
- Admin panel for product & order management
- Responsive UI with performance optimization

**Tech:** Next.js, Tailwind  

---

### 🏢 GSMGPT Systems

#### 📚 Academy Platform
- Structured UI for educational content delivery

#### 🍏 Apple ID Marketplace
- Marketplace UI with clean user flows
- Responsive and optimized interface

#### 📡 MiStatus – Server Monitoring Dashboard
- Real-time system monitoring dashboard
- Live server status visualization

#### ⚙️ Admin & Operator Panels
- Scalable dashboards for internal systems
- Reusable UI components for complex workflows
- Improved operational efficiency

---

### ⚽ Football Bartar (IRIB)
- Contributed to frontend of a **high-traffic media platform**

---

## 📊 GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Tocka69&show_icons=true&hide_border=true&bg_color=0d1117&title_color=6AAFE6&icon_color=6AA6C7&text_color=c9d1d9" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Tocka69&layout=compact&hide_border=true&bg_color=0d1117&title_color=6AAFE6&text_color=c9d1d9" height="160"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Tocka69&hide_border=true&background=0d1117&ring=5AA6C7&fire=5AA6C7&currStreakLabel=6AAFE6&dates=c9d1d9" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Tocka69&bg_color=0d1117&line=5AA6C7&point=5AA6C7&color=c9d1d9&hide_border=true" />
</p>

---

## 🏆 Skills

<p align="center">
  <img src="https://img.shields.io/badge/React-0d1117?style=for-the-badge&logo=react&logoColor=6AAFE6"/>
  <img src="https://img.shields.io/badge/Next.js-0d1117?style=for-the-badge&logo=next.js&logoColor=c9d1d9"/>
  <img src="https://img.shields.io/badge/TypeScript-0d1117?style=for-the-badge&logo=typescript&logoColor=6AAFE6"/>
  <img src="https://img.shields.io/badge/Tailwind-0d1117?style=for-the-badge&logo=tailwindcss&logoColor=6AAFE6"/>
  <img src="https://img.shields.io/badge/Map-0d1117?style=for-the-badge&logo=mapbox&logoColor=5AA6C7"/>
</p>

---

## 🌐 Connect

<p align="center">
  <a href="https://github.com/Tocka69"><img src="https://img.shields.io/badge/GitHub-0d1117?style=for-the-badge&logo=github&logoColor=6AAFE6"/></a>
  <a href="www.linkedin.com/in/amirmohamad-kazemi-6a94a63ba"><img src="https://img.shields.io/badge/LinkedIn-0d1117?style=for-the-badge&logo=linkedin&logoColor=5AA6C7"/></a>
  <a href="mailto:am9268510@gmail.com"><img src="https://img.shields.io/badge/Email-0d1117?style=for-the-badge&logo=gmail&logoColor=6AAFE6"/></a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=140&section=footer&color=0:2e3440,100:5AA6C7"/>
</p>
